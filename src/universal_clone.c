...
void fetch_usb_string(int fd, uint8_t index, char *out, int out_len) {
    ...
    struct usbdevfs_ctrltransfer ctrl = {
        ...
        .wValue = (3 << 8) | index, .wIndex = 0x0409,
        .wLength = sizeof(data), .data = data, .timeout = 1000
    };
    
    if (ioctl(fd, USBDEVFS_CONTROL, &ctrl) > 2 && data[1] == 3) {
        int len = data[0]; int j = 0;
        for (int i = 2; i < len && j < out_len - 1; i += 2) out[j++] = data[i];
        out[j] = '\0';
    } else { snprintf(out, out_len, "Generic_Device"); }
}
...
int main(int argc, char **argv) {
    ...
    char dev_dir[512], cmd[512], desc_path[512];
    snprintf(dev_dir, sizeof(dev_dir), "%s/fake_usb/sys/bus/usb/devices/1-1", home);

    snprintf(cmd, sizeof(cmd), "rm -rf %s/fake_usb && mkdir -p %s && mkdir -p %s/fake_usb/dev/bus/usb/001", home, dev_dir, home); system(cmd);
    ...
    write_str(dev_dir, "busnum", "1"); write_str(dev_dir, "devnum", argv[2]);
    write_str(dev_dir, "bConfigurationValue", "1"); write_str(dev_dir, "speed", "480");
    ...
    int offset = 18, current_config = 1;
    while (offset < len) {
        uint8_t d_len = buf[offset], d_type = buf[offset + 1];
        if (d_len == 0) break;
        if (d_type == 2) current_config = buf[offset + 5];
        else if (d_type == 4) {
    ...
    write_str(root_dir, "busnum", "1"); write_str(root_dir, "devnum", "1");
